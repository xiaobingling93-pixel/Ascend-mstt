...
Step 3: 配置Msmonitor日志路径(npu-monitor功能可选操作)
...
```python
    ...
    if __name__ == '__main__':
        ...
        for i in range(step_num):
            # 模型训练
            train(net)
            # 调用step方法实现npu trace dump以或npu monitor功能
            dp.step()
```
...
该方式的使能方式与动态profiling自定义for循环方式一致，唯一区别是将step()方法适配在step_begin、step_end回调函数中。
