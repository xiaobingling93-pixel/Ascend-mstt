  ...
  resources: {
    en: {
      translation: {
        ...
        generate_matching_configuration_file_tooltip:
          'After completing manual matching, clicking Save Matching Node Information will save the matched node relationships to the configuration file without persisting the original file. If this is the first save, a new file will be created named: [Current File Name].vis.config',
        save_tooltip:
          'Note: After completing the matching process, you must click the Save button to update the modified data to the file. Otherwise, the changes will not take effect.',
        module_or_operators: 'Module or Operators',
        unexpanded_nodes: 'Unexpanded Nodes',
        api_list: 'Api List',
        multi_collection: 'Multi Collection',
        unexpanded_nodes_tooltip:
        ...
        expand_fail_no_match_nodes:
          'Expansion failed: No matching nodes found for the current node or its parent node.',
        retrieve_precision_menu_fail:
          'Failed to retrieve precision menu. Please check the data in the toggleCheckbox and vis files.',
    ...
    'zh-CN': {
      translation: {
        ...
        generate_matching_configuration_file_tooltip:
          '手动匹配结束后，点击保存匹配节点信息，会将已匹配的节点对应关系保存到配置文件中，不会持久原始文件，如果是初次保存，会新建一个文件，文件名称为：[当前文件名].vis.config',
