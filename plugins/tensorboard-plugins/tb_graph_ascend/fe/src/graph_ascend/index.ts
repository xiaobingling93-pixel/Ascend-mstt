...
class TfGraphDashboard extends LegacyElementMixin(PolymerElement) {
    ...
    <template is="dom-if" if="[[!progressData.done]]">
      <div class="progress-bar">
        <label class="text-secondary"
          >[[progressData.title]] [[progressData.progress]] %</label
        >
        <vaadin-progress-bar
  ...
  updateGraphData = () => {
    ...
    if (
      this.currentSelection?.run !== this.selection?.run ||
      this.currentSelection?.tag !== this.selection?.tag
    ) {
  ...
  loadGraphConfig = async (run, tag) => {
    const { success, data } = await this.useGraphAscend.loadGraphConfig(
      run,
      tag
    );
    ...
    if (success) {
      ...
      if (microstepsCount) {
        const microstepsArray = Array.from(
          { length: microstepsCount + 1 },
          (_, index) => ({
  ...
  loadGraphAllNodeList = async (run, tag, microStep) => {
    const { success, data } = await this.useGraphAscend.loadGraphAllNodeList(
      run,
      tag,
      microStep
    );
  ...
  initGraphBoard = () => {
    (this.shadowRoot?.querySelector("#graph-board") as any)?.initGraphHierarchy(
      this.jumpToNode
    );
    if (this.jumpToNode) {
