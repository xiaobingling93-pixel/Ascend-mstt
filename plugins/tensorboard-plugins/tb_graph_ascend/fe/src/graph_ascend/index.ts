...
class TfGraphDashboard extends LegacyElementMixin(PolymerElement) {
  static readonly template = html`
    ...
    >
      <div class="file-list-error">
        ...
        <div>
          非授权路径访问可能存在信息泄露和文件内容篡改。
          文件过大或格式异常，可能导致性能问题或服务中断。路径中存在软链接或权限不当，可能存在越权访问和数据篡改风险。
        </div>
  ...
  updateGraphData = () => {
    ...
    const isFileChange =
      this.currentSelection?.run !== this.selection?.run || this.currentSelection?.tag !== this.selection?.tag;
    const isDBChange =
      this.currentSelection?.rank !== this.selection?.rank || this.currentSelection?.step !== this.selection?.step;
    if (isFileChange) {
  ...
  loadDBGraphData = async (metaData: SelectionType, isInitDB: boolean = false) => {
    if (isInitDB) {
      this.progreesLoading(this.t('initializing_database'), this.t('please_wait'), {
        progress: 10,
        progressValue: 10,
        done: false,
      });
    ...
    this.progreesLoading(this.t('initializing_graph'), this.t('please_wait'), {
      progress: 90,
      progressValue: 90,
      done: false,
    });
    this.initGraphBoard(); // 先读取配置，再加载图,顺序很重要
    this.progreesLoading(this.t('initialization_complete'), this.t('please_wait'), {
      progress: 100,
      progressValue: 100,
      done: true,
    });
  ...
  loadJSONGraphData = async (metaData: SelectionType) => {
    ...
    this.eventSource = new EventSource(
      `loadGraphData?run=${metaData.run}&tag=${metaData.tag}&type=${metaData.type}&lang=${metaData.lang}`,
    );
