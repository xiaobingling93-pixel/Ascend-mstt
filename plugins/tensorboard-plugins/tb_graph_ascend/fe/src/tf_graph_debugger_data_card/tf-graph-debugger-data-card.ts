...
class TfGraphDebuggerDataCard extends LegacyElementMixin(PolymerElement) {
   ...
   * Apparently, the paper-slider lacks a mixin for those padding values. */
    ...
    <paper-material elevation="1" class="card health-pill-legend">
      <div class="title">Enable all (not just sampled) steps. Requires slow disk read.</div>
      <paper-toggle-button id="enableAllStepsModeToggle" checked="{{allStepsModeEnabled}}"> </paper-toggle-button>
      <h2>
        ...
        <template is="dom-if" if="[[!allStepsModeEnabled]]"> [[_currentStepDisplayValue]] </template>
        <paper-spinner-lite
          active
      ...
      <h2>
        Health Pill
        <template is="dom-if" if="[[healthPillValuesForSelectedNode]]"> Counts for Selected Node </template>
        <template is="dom-if" if="[[!healthPillValuesForSelectedNode]]"> Legend </template>
      </h2>
      <template is="dom-repeat" items="[[healthPillEntries]]">
        <div class="health-pill-entry">
          ...
          <div class="tensor-count">[[_computeTensorCountString(healthPillValuesForSelectedNode, index)]]</div>
        </div>
      </template>
      ...
      <template is="dom-if" if="[[!_hasDebuggerNumericAlerts(debuggerNumericAlerts)]]">
        <p class="no-numeric-alerts-notification">
          No numeric alerts so far. That is likely good. Alerts indicate the presence of NaN or (+/-) Infinity values,
  ...
  get healthPillValuesForSelectedNode(): unknown[] | null {
    let nodeNamesToHealthPills = this.nodeNamesToHealthPills;
    let healthPillStepIndex = this.healthPillStepIndex;
    let selectedNode = this.selectedNode;
    let allStepsModeEnabled = this.allStepsModeEnabled;
    let areHealthPillsLoading = this.areHealthPillsLoading;
  ...
  get _currentStepDisplayValue(): any {
    ...
    let healthPillStepIndex = this.healthPillStepIndex;
    let allStepsModeEnabled = this.allStepsModeEnabled;
    let specificHealthPillStep = this.specificHealthPillStep;
    let areHealthPillsLoading = this.areHealthPillsLoading;
    ...
    for (let nodeName in nodeNamesToHealthPills) {
      if (Object.prototype.hasOwnProperty.call(nodeNamesToHealthPills, nodeName)) {
        // All nodes have the same number of steps stored, so only examine 1 node.
        // We do not directly index into the nodeNamesToHealthPills object because we do not have a key.
  ...
  get _biggestStepEverSeen(): number {
    ...
    for (let nodeName in nodeNamesToHealthPills) {
      if (Object.prototype.hasOwnProperty.call(nodeNamesToHealthPills, nodeName)) {
        ...
        let healthPills = nodeNamesToHealthPills[nodeName];
  ...
  _updateAlertsList(): void {
    ...
    for (let i = 0; i < debuggerNumericAlerts.length; i++) {
      ...
      let miniHealthPill = document.createElement('div');
      miniHealthPill.classList.add('mini-health-pill');
      let miniHealthPillTd = document.createElement('td');
      ...
      if (alert.neg_inf_event_count) {
        let negativeInfCountSection = document.createElement('div');
      ...
      if (alert.pos_inf_event_count) {
        let positiveInfCountSection = document.createElement('div');
      ...
      if (alert.nan_event_count) {
        let nanCountSection = document.createElement('div');
  ...
  _addOpExpansionListener(clickableElement: any, tensorName: any): void {
    clickableElement.addEventListener('click', () => {
      ...
      // associated node.
      let nameOfNodeToSelect = tf_graph_render.expandUntilNodeIsShown(
      ...
      let previousScrollFromBottom: any;
      let graphInfoCard = document.querySelector('tf-graph-info#graph-info') as any;
      if (graphInfoCard) {
        previousScrollFromBottom = graphInfoCard.scrollHeight - graphInfoCard.scrollTop;
      }
      // Update the selected node within graph logic.
      let previousSelectedNode = this.selectedNode;
      ...
      let scrollToOriginalLocation = (): void => {
