...
export class RequestManager {
  ...
  public requestWithOptions(url: string, requestOptions: RequestOptions): Promise<any> {
   ...
   */
  private promiseWithRetries(url: string, maxRetries: number, requestOptions: RequestOptions): any {
    let success = (x): any => x;
    let failure = (x): any => {
      if (maxRetries > 0) {
...
function formDataFromDictionary(postData: PostData): FormData {
