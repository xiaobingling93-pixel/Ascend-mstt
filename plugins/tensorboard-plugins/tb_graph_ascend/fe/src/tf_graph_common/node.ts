...
function traceAllInputsOfOpNode(
  ...
  renderGraphInfo: render.RenderGraphInfo,
  startNode: OpNode,
  allTracedNodes: Record<string, unknown>,
): any {
 ...
 * @private
 */
function _findVisibleParentsFromOpNodes(renderGraphInfo, nodeNames: string[]): { [nodeName: string]: tf_graph.Node } {
  let visibleParents: {
...
function _markParentsOfNodes(
  ...
  _.forOwn(visibleNodes, (nodeInstance: Node) => {
    ...
    while (currentNode.name !== tf_graph.ROOT_NAME) {
      ...
      if (shouldSkip(renderedElementSelection)) {
 ...
 * @returns Node
 */
export function getVisibleParent(renderGraphInfo: render.RenderGraphInfo, currentNode: tf_graph.Node): tf_graph.Node {
  let found = false;
...
export function buildGroupForAnnotation(
  ...
  let annotationGroups = container
    ...
    .data(annotationData.list, (dNode) => {
      return dNode.node.name;
    });
  annotationGroups
    .enter()
    .append('g')
    ...
    .each(function (a) {
      ...
      let metaedge = a.renderMetaedgeInfo?.metaedge;
 ...
 */
function annotationToClassName(annotationType: render.AnnotationType): string | null {
  return (render.AnnotationType[annotationType] || '').toLowerCase() || null;
}
function buildShapeForAnnotation(aGroup, a: render.Annotation): void {
...
function addAnnotationLabelFromNode(aGroup, a: render.Annotation): void {
  let namePath = a.node.name.split('/');
  let text = namePath[namePath.length - 1];
  addAnnotationLabel(aGroup, text, a, null);
}
function addAnnotationLabel(aGroup, label: string, a: render.Annotation, additionalClassNames): void {
  ...
  let txtElement = aGroup
    ...
    .text(label);
  enforceLabelWidth(txtElement, -1, FontSizeInPx.Annotation.LABEL);
}
function addInteractionForAnnotation(
  ...
  if (
    ...
    selection.on('contextmenu', contextmenu.getMenu(sceneElement, d));
  }
}
/**
 ...
 * @param sceneElement <tf-graph-scene> polymer element.
 */
function update(aGroup, d: render.RenderNodeInfo, a: render.Annotation, sceneElement): void {
  let cx = layout.computeCXPositionOfNodeShape(d);
