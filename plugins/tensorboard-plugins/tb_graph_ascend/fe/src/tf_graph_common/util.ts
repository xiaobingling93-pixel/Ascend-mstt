 ...
 */
export function time<T>(msg: string, task: () => T, debugEventId?: GraphDebugTimingEventId): T {
  let start = Date.now();
  let result = task();
...
export function getSubtaskTracker(
  ...
  return {
    setMessage: function (progressMsg): void {
    ...
    updateProgress: function (incrementValue): void {
...
export function runAsyncPromiseTask<T>(
  ...
  return new Promise((resolve, reject) => {
    let handleError = function (e): void {
      // Errors that happen inside asynchronous tasks are
      // reported to the tracker using a user-friendly message.
      tracker.reportError(`Failed${msg}`, e);
      reject(e);
...
export function escapeQuerySelector(querySelector: string): string {
  return querySelector.replace(/(?<specialChar>[:.\[\],/\\\(\)])/g, '\\$<specialChar>');
}
/**
...
export function removeCommonPrefix(strings: string[]): string[] {
  ...
  let minLength = _.min(_.map(strings, (str) => str.length)) as number;
  while (true) {
    index++;
 ...
 */
export function computeHumanFriendlyTime(timeInMicroseconds: number): string {
  let timeDifferenceInMs = Number(new Date()) - Number(new Date(timeInMicroseconds / 1000));
...
export class Dispatcher<EventType = any> {
  private eventTypeToListeners = new Map<EventType, Array<() => void>>();
  ...
  dispatchEvent(eventType: EventType, payload?: any): void {
    for (const listener of this.getListeners(eventType)) {
      listener(payload);
    }
  }

  private getListeners(eventType): any {
