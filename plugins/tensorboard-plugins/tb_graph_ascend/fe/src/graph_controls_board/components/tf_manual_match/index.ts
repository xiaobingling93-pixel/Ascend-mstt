...
class Legend extends PolymerElement {
    ...
    <vaadin-details class="vaadin-details" summary="节点匹配" opened>
      ...
      <div class="unmatched-node">
        ...
        <div class="match-button">
          <vaadin-button theme="secondary small" on-click="_addMatchedNodesLink"
            >点击匹配</vaadin-button
          >
        </div>
      </div>
      <div class="matched-node">
        ...
        <div class="match-button">
          <vaadin-button
            ...
            on-click="_deletelMatchedNodesLink"
            >取消匹配</vaadin-button
          >
        </div>
      </div>
  ...
  _observeSelectedNode(): void {
    ...
    if (this.selectedNode.startsWith(NPU_PREFIX)) {
      this.set(
        "selectedNpuUnMatchedNode",
        this.selectedNode.replace(NPU_PREFIX, "")
      );
      const selectedNpuMatchedNode = this.selectedNode.replace(NPU_PREFIX, "");
      this.set("selectedNpuMatchedNode", selectedNpuMatchedNode);
      this.set(
        "selectedBenchMatchedNode",
        this.npuMatchedNodeList?.[selectedNpuMatchedNode]
      );
    } else if (this.selectedNode.startsWith(BENCH_PREFIX)) {
      this.set(
        "selectedBenchUnMatchedNode",
        this.selectedNode.replace(BENCH_PREFIX, "")
      );
      const selectedBenchMatchedNode = this.selectedNode.replace(
        BENCH_PREFIX,
        ""
      );
      this.set("selectedBenchMatchedNode", selectedBenchMatchedNode);
      this.set(
        "selectedNpuMatchedNode",
        this.benchMatchedNodeList?.[selectedBenchMatchedNode]
      );
    }
  }
  ...
  _changeNpuMatchedNode = (): void => {
    if (this.isCompareGraph) {
      ...
      this.set(
        "selectedBenchMatchedNode",
        this.npuMatchedNodeList[this.selectedNpuMatchedNode]
      );
      this.set("selectedNode", node);
  ...
  _changeBenchMatchedNode = (): void => {
    if (this.isCompareGraph) {
      ...
      this.set(
        "selectedNpuMatchedNode",
        this.benchMatchedNodeList[this.selectedBenchMatchedNode]
      );
      this.set("selectedNode", node);
  ...
  async _deletelMatchedNodesLink(): Promise<void> {
    ...
    const { success, data, error } =
      await this.useMatched.deleteMatchedNodesLink(
    ...
    if (success) {
      ...
      const updateHierarchyData = new CustomEvent("updateHierarchyData", {
        bubbles: true,
        composed: true,
      });
      const porcessedNodeNum = Math.abs(
        npuUnMatchNodes?.length - this.npuUnMatchedNodes?.length
      );
      ...
      this.set("selectedBenchMatchedNode", "");
      Notification.show(
        `取消成功：取消节点数 ${porcessedNodeNum} 个,对应节点状态已更新`,
        {
          position: "middle",
          duration: 4000,
          theme: "success",
  ...
  _addMatchedNodesLinkByConfigFile = async (): Promise<void> => {
    ...
    const { success, data, error } =
      await this.useMatched.addMatchedNodesLinkByConfigFile(
  ...
  async _addMatchedNodesLink(): Promise<void> {
    ...
    if (success) {
      ...
      this.set("selectedBenchUnMatchedNode", "");
      Notification.show(
        `匹配成功：匹配节点数 ${porcessedNodeNum} 个,对应节点状态已更新`,
        {
          position: "middle",
          duration: 4000,
          theme: "success",
  ...
  async _saveMatchedRelations(): Promise<void> {
    ...
    const { success, data, error } = await this.useMatched.saveMatchedRelations(
      this.selection
    );
    if (success) {
      const configFile = data;
      const matchedConfigFiles = [
        ...new Set(["未选择", configFile, ...this.matchedConfigFiles]),
      ];
      this.set("matchedConfigFiles", matchedConfigFiles);
      Notification.show(
        `操作成功:文件已生成到当前目录下，文件名称为${configFile}`,
        {
          position: "middle",
          duration: 3000,
          theme: "success",
