...
export function addHashListener(fn: () => void): ListenKey {
  const key = new ListenKey(fn);
  hashListeners.add(key);
  return key;
}
export function addStorageListener(fn: () => void): ListenKey {
  const key = new ListenKey(fn);
  storageListeners.add(key);
  return key;
}
export function fireStorageChanged(): void {
  storageListeners.forEach((listenKey) => listenKey.listener());
}
export function removeHashListenerByKey(key: ListenKey): void {
  hashListeners.delete(key);
}
export function removeStorageListenerByKey(key: ListenKey): void {
