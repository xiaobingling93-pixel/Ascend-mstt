...
const useGraph = (): UseGraphType => {
  const preProcessData = (
    ...
    virtualNodes.forEach((d) => {
      ...
      while (node?.parentNode) {
        const parent = data.find(
          (d) =>
            node?.parentNode ===
            d.name?.replace(new RegExp(`^(${NPU_PREFIX}|${BENCH_PREFIX})`), "")
        );
        if (parent) {
    ...
    const renderData = virtualNodes.map((d) => {
      let precisionColor = isOverflowFilter
        ? getOverflowColor(d)
        : getPrecisionColor(d, colors, graphType);
      let strokeColor;
      ...
      if (
        d.nodeType === NODE_TYPE.API_LIST ||
        d.nodeType === NODE_TYPE.MULTI_COLLECTION
      ) {
      ...
      return {
        ...
        stroke: strokeColor,
        strokeWidth:
          d.name === selectedNode ? SELECTED_STROKE_WIDTH : STROKE_WIDTH,
      };
    });
    return renderData;
  };
    ...
    if (isEmpty(node.matchedNodeLink)) {
      return (
        Object.keys(colors).find(
          (color) => colors[color].value === "无匹配节点"
        ) || NO_MATCHED_NODE_COLOR
      );
    }
  ...
  const bindInnerRect = (container, data) => {
    ...
    const innnerReact = container
      .selectAll(".inner-rect")
  ...
  const bindOuterRect = (container, data) => {
    // 绑定数据到 outerReact 元素
    const outerReact = container
      .selectAll(".outer-rect")
      .data(data, (d: any) => d.name);
    outerReact
      .transition()
      .duration(DURATION_TIME)
  ...
  const changeNodeExpandState = async (
    nodeInfo: any,
    metaData: any
  ): Promise<any> => {
    try {
