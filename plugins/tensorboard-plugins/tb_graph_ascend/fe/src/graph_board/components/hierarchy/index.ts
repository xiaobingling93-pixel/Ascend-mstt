...
class Hierarchy extends PolymerElement {
  ...
  colors: PreProcessDataConfigType["colors"] =
    {} as PreProcessDataConfigType["colors"];
  ...
  @property({ type: Object })
  hightLightMatchedNode: (matchedNodes, graphType) => void = (
    matchedNodes,
    graphType
  ) => {};
  ...
  async initHhierarchy(selectedNode) {
    ...
    if (success) {
      ...
      const hierarchyData = Object.values(
        hierarchyObject
      ) as Array<HierarchyNodeType>;
      // 清空container下面的所有子元素
      if (this.container) {
        this.container.innerHTML = "";
        d3.select(this.container as HTMLElement).attr(
          "transform",
          "translate(32,32) scale(1.8)"
        );
      }
  ...
  initMinimap = () => {
    ...
    const mainZoom = d3.zoom().on("zoom", () => {
      ...
      if (this.container) {
        d3.select(this.container as HTMLElement).attr(
          "transform",
          (d3 as any).event.transform.toString()
        );
      }
      this.renderGraph(this.hierarchyData, this.selectedNode);
  ...
  renderGraph(data, selectedNode, transform = this.getContainerTransform()) {
    ...
    // 数据预处理
    const prefix =
      this.graphType === "Single"
        ? ""
        : this.graphType === "NPU"
          ? NPU_PREFIX
          : BENCH_PREFIX;
    selectedNode = selectedNode.startsWith(prefix)
      ? selectedNode
      : `${prefix}${selectedNode}`; // 加上前缀
    const config = {
      colors: this.colors,
      isOverflowFilter: this.isOverflowFilter,
      graphType: this.graphType,
    };
    const renderData = this.useGraph.preProcessData(
      data,
      selectedNode,
      config,
      transform
  ...
  async changeSelectNode(selectedNode) {
    ...
    const selectedNodeType = selectedNode.startsWith(NPU_PREFIX)
      ? "NPU"
      : "Bench";
    if (this.graphType !== "Single" && selectedNodeType !== this.graphType) {
      return;
    } // 如果选中的节点类型和当前图类型不一致，则不处理
    const nodeName = selectedNode.replace(
  ...
  bindEventLisetener = () => {
    ...
    const cleanChangeNodeExpandStateEvent = this.bindChangeNodeExpandStateEvent(
      this.container
    );
  ...
  bindUpdateHierarchyDataEvent() {
    const onUpdateHierarchyDataEvent = async () => {
      ...
      const { success, data, error } = await this.useGraph.updateHierarchyData(
        this.graphType
      );
  ...
  bindContextMenuEvent() {
    ...
    const onContextMenuItemSelectedEvent = (
      event: ContextMenuItemSelectedEvent
    ) => {
      event.preventDefault();
      ...
      if (item.type === EXPAND_MATCHED_NODE) {
        ...
        const tempSelectedNode = this.selectedNode;
        let nodeName = tempSelectedNode?.replace(
          new RegExp(`^(${NPU_PREFIX}|${BENCH_PREFIX})`),
          ""
        );
        let selectedNode = this.hierarchyObject[nodeName];
        while (
          isEmpty(selectedNode?.matchedNodeLink) &&
          selectedNode?.parentNode
        ) {
          nodeName = selectedNode.parentNode?.replace(
            new RegExp(`^(${NPU_PREFIX}|${BENCH_PREFIX})`),
            ""
          );
        ...
        if (!isEmpty(selectedNode?.matchedNodeLink)) {
          let matchedNodeName =
            selectedNode.matchedNodeLink[
              selectedNode.matchedNodeLink.length - 1
            ];
          const matchedPrefix =
            this.graphType === "NPU" ? BENCH_PREFIX : NPU_PREFIX;
            ...
            const prefix =
              this.graphType === "Single"
                ...
                : this.graphType === "NPU"
                  ...
                  : BENCH_PREFIX;
            return {
              component: this.useGraph.createComponent(
                `rank${rankId}`,
                precision_index,
                this.colors
              ),
    ...
    const throttleContextMenuItemSelectedEvent = throttle(
      onContextMenuItemSelectedEvent,
      16
    );
    const throttleContextMenuEvent = throttle(onContextmenuEvent, 16);
    contextMenu.addEventListener(
      "item-selected",
      throttleContextMenuItemSelectedEvent as any
    );
  ...
  bindChangeNodeExpandStateEvent(container) {
    const onDoubleClickNodeEvent = async (event) => {
      ...
      if (
        this.hierarchyObject[nodeInfo.nodeName || ""]?.nodeType ===
        NODE_TYPE.UNEXPAND_NODE
      ) {
        return;
      }
      await this.changeNodeExpandState(nodeInfo);
  ...
  changeNodeCenter(nodeName) {
    ...
    const newY =
      clientHeight / 2 - (selectedNode?.y * initialTransform.scale + 7.5) - 100;
    changeGraphPosition(
      this.container as HTMLElement,
      newX,
      newY,
  ...
  async changeNodeExpandState(nodeInfo) {
    this.set("loading", true);
    const { success, data, error } = await this.useGraph.changeNodeExpandState(
      nodeInfo,
      this.selection
    );
    this.set("loading", false);
