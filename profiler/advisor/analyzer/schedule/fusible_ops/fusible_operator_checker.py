...
from profiler.advisor.utils.utils import convert_to_float_with_warning
...
class FusibleOperatorChecker:
    ...
    def __init__(self, **kwargs):
        ...
        self._init_rule()
        self.headers = [
            "start index", "end index", "wall duration(us)", "NPU duration(us)", "MTE time(us)", "occurrences",
    ...
    def check_fusible_operator(self, profiling_dataset: ProfilingDataset):
        ...
        self.step_duration = convert_to_float_with_warning(tasks[-1].task_start_time) + \
                             convert_to_float_with_warning(tasks[-1].task_duration) - \
                             convert_to_float_with_warning(tasks[0].task_start_time)
        ...
        for index, task in enumerate(tasks):
            ...
            for i in range(1, self.max_length):
                ...
                total_time = convert_to_float_with_warning(new_task.task_start_time) - start_time + \
                             convert_to_float_with_warning(new_task.task_duration)
    ...
    def check_tasks(self, profiling_dataset: ProfilingDataset):
        if not hasattr(profiling_dataset, "op_summary"):
            ...
            return False
        task = profiling_dataset.op_summary.op_list[0]
