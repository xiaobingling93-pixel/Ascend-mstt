...
class TimelineAdviceBase(AdviceBase):
    ...
    def path_check(self):
        ...
        if not os.path.exists(self.collection_path):
            logger.error("[ERROR] Path: {} is not exist.".format(self.collection_path))
            return False
        if os.path.isdir(self.collection_path) and self.collection_path.endswith("ascend_pt"):
            ...
            if not os.path.exists(self.trace_view_path):
                logger.error("[ERROR] trace_view.json is not exist in the Path: {}."\
                             .format(os.path.join(self.collection_path, "ASCEND_PROFILER_OUTPUT")))
                return False
            ...
            return False
        logger.info("[INFO] Start to analyse the target file: {}".format(self.trace_view_path))
        return True
