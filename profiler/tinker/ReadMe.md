...
Tinker大模型并行策略自动寻优系统（简称Tinker），根据提供的训练脚本，进行约15分钟的单节点NPU性能测量，推荐高性能并行策略训练脚本。
...
1. 配置训练框架相关环境：请确保使用当前环境已安装/配置`CANN套件`、`训练框架依赖库`、`megatron`、`mindspeed加速库`等依赖（Tinker运行需求训练框架能力）；
   ...
   python, driver, firmware, CANN, torch, torch_npu以及其他依赖安装步骤请参考训练框架（MindSpeed-LLM）的[README](https://gitcode.com/Ascend/MindSpeed-LLM/blob/master/docs/zh/introduction.md)。
...
3. 导入MindSpeed-LLM框架路径`export ML_PATH=/XX/XX/XX/MindSpeed-LLM`

4. 克隆mstt库`git clone https://gitcode.com/Ascend/mstt.git`
...
`python /xxx/tinker/tinker_auto_parallel.py --mode all --config XXX.json ...`
...
- `--version`: 指定使用的MindSpeed-LLM框架版本，可选`1.2` `1.1` `1.0` `1.0.0` `2.0.0`分别对应`1.0.RC3` `1.0.RC2` `1.0.RC1` `1.0.0` `2.0.0`(可选，默认值1.1)
...
1. 性能测量中Tinker对训练脚本依赖的说明
   ...
   Tinker测量数据时会处理、储存指定的训练脚本并部分运行。请确保正确填写`TOKENIZER_PATH`，`DATA_PATH`（可参考MindSpeed-LLM的[README](https://gitcode.com/Ascend/MindSpeed-LLM/blob/master/docs/zh/introduction.md)填写）。Tinker在处理脚本时，会删除torchrun逻辑、并行策略、预训练权重存取相关命令行参数，然后将其他模型、训练相关参数存放于`GPT_ARGS`中。具体剔除的参数包括：
...
2. 策略寻优中设定内存上限`-mem`推荐值的解释：当前推荐值为经验值。Tinker仅预测torch算子占用内存开销，因此设定`-mem`时需注意预留CANN、HCCL等组件的内存开销，并避免极端内存使用带来的反复内存搬移，导致性能降低。
