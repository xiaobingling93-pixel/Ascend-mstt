...
class TestGpuProfilingParser(unittest.TestCase):
    ...
    def test_calculate_performance_time_when_valid_input(self):
        with patch("compare_backend.profiling_parser.base_profiling_parser.BaseProfilingParser.__init__"), \
            ...
            self.assertEqual(res._result_data.overall_metrics.cube_time, 0)
            self.assertEqual(res._result_data.overall_metrics.cube_num, 0)
            self.assertEqual(res._result_data.overall_metrics.fa_time_fwd, 0)
            self.assertEqual(res._result_data.overall_metrics.fa_num_fwd, 0)
            self.assertEqual(res._result_data.overall_metrics.fa_time_bwd, 0)
            self.assertEqual(res._result_data.overall_metrics.fa_num_bwd, 0)
            self.assertEqual(res._result_data.overall_metrics.vec_time, 0)
            self.assertEqual(res._result_data.overall_metrics.vec_num, 0)  # cun yi
