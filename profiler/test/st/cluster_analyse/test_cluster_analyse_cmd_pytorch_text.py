...
class TestClusterAnalyseCmdPytorchText(TestCase):
    ...
    OUTPUT_DATA = ""
    RESULT_CSV = "cluster_step_trace_time.csv"
    RESULT_BANDWIDTH = "cluster_communication_matrix.json"
    RESULT_COMMUNICATION = "cluster_communication.json"
    COMMAND_SUCCESS = 0
    ...
    def setup_class(self):
        ...
        if self.RESULT_CSV not in files:
            self.assertEqual(False, True, 
                             msg = f"PyTorch text result {self.RESULT_CSV} not find.")
        if self.RESULT_BANDWIDTH not in files:
            self.assertEqual(False, True, 
                             msg = f"PyTorch text result {self.RESULT_BANDWIDTH} not find.")
        if self.RESULT_COMMUNICATION not in files:
            self.assertEqual(False, True, 
                             msg = f"PyTorch text result {self.RESULT_COMMUNICATION} not find.")
    ...
    def test_trace_time_compare(self):
        ...
        """
        df = pd.read_csv(os.path.join(self.OUTPUT_DATA, self.RESULT_CSV), sep = ',', header = 0)
        headers = ["Step", "Type", "Index", "Computing", "Communication(Not Overlapped)", "Overlapped", "Communication", "Free", 
