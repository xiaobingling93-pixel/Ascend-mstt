...
def save_huggingface_llama(args, model, model_args):
    ...
    for name_param_m in model.named_parameters():
        layer_num = name_param_m[0].split(".")[3] if len(
            name_param_m[0].split(".")) > 3 else name_param_m[0].split(".")[1]
        ...
        if name_param_m[0] == f"language_model.encoder.layers.{layer_num}.self_attention.query_key_value.bias":
            bias_weight = name_param_m[1].reshape(
                ng, repeats +
                2, name_param_m[1].shape[0] // ng // (repeats + 2)
            )
    ...
    for filename in file_list:
            ...
            try:
                ...
                if 0 <= index1 <= index2 <= 99999:
                    ...
                    file_path = os.path.join(
                        args.output_model_dir, formatted_filename)
                    hf_model = torch.load(
                    ...
                    output_file_path = os.path.join(
                        output_mg2hg_path, formatted_filename)
        ...
        elif (filename.endswith('.json') or filename.endswith('.mode')) and 'safetensors' not in filename:
...
def save_model_checkpoint(queue, args):
    ...
    if md.make_vocab_size_divisible_by is not None:
        sys.argv.extend(['--make-vocab-size-divisible-by',
                        str(md.make_vocab_size_divisible_by)])
    if md.output_layer:
    ...
    def get_models(count, dtype, pre_process, post_process):
        models = [model_provider(pre_process, post_process).to(
            dtype) for _ in range(count)]
        return models
    ...
    models = get_models(args.target_tensor_parallel_size,
                        md.params_dtype, True, post_process)
    for tp_rank, model in enumerate(models):
        ...
        if orig_word_embed_n_w is not None:
            model.language_model.embedding.word_embeddings.norm.weight.data.copy_(
                orig_word_embed_n_w)
            model.language_model.embedding.word_embeddings.norm.bias.data.copy_(
                orig_word_embed_n_b)
    ...
    for pp_rank in range(args.target_pipeline_parallel_size):
        ...
        if pp_rank > 0:
            ...
            models = get_models(args.target_tensor_parallel_size,
                                md.params_dtype, False, post_process)
        ...
        for layer in range(encoder_layer_num):
            ...
            qkv_weight = torch.chunk(
                qkv_org, args.target_tensor_parallel_size, dim=0)
            ...
            for tp_rank in range(args.target_tensor_parallel_size):
                ...
                layer_encoder.self_attention.query_key_value.weight.data.copy_(
                    qkv_weight[tp_rank])
                ...
                if md.linear_bias:
                    layer_encoder.self_attention.query_key_value.bias.data.copy_(
                        qkv_bias[tp_rank])
            ...
            if msg != "done" and msg["name"] == "lm head":
                ...
                for tp_rank in range(args.target_tensor_parallel_size):
                    ...
                    models[tp_rank].lm_head.dense.bias.data.copy_(
                        lm_head_dense_bias)
                    models[tp_rank].lm_head.norm.weight.data.copy_(
                        lm_head_norm_weight)
                    if md.norm_has_bias:
                        models[tp_rank].lm_head.norm.bias.data.copy_(
                            lm_head_norm_bias)
                check_message(msg)
                msg = queue_get()
