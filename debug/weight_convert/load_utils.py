...
def _load_checkpoint(queue, args):
    ...
    # Send embeddings
    message_word_embedding = []
    for tp_rank in range(tp_size):
        message_word_embedding.append(models[tp_rank].language_model.embedding.word_embeddings.weight.data)
    message = {"word embeddings": torch.cat(message_word_embedding, dim=0)}
    ...
    if md.output_layer:
        message_weight = []
        for tp_rank in range(tp_size):
            message_weight.append(models[tp_rank].language_model.output_layer.weight.data)
        message = {"weight": torch.cat(message_weight, dim=0)}
