...
from .algorithm import compare_core
from ..common.file_check_util import FileOpen
from .utils import logger_info, logger_error, DispatchException, CompareConst
...
def get_json_contents(file_path):
    ...
    try:
        ...
        logger_error('Failed to load "%s". %s' % (file_path, str(error)))
        raise DispatchException(1, "failed to load json.") from error
    if not isinstance(json_obj, dict):
        logger_error('Json file %s, content is not a dictionary!' % file_path)
    ...
    def write_detail_csv(self, test_result):
        ...
        if isinstance(fwd_result, list):
            for i, test_subject in enumerate(fwd_result):
                ...
                test_subject = ["{:.{}f}".format(item, 14) if isinstance(item, float) else item for item in test_subject]
