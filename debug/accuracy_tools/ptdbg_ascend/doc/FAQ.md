...
### 14. 添加ptdbg_ascend工具后截取操作报错：`IndexError: too many indices for tensor of dimension x` 或 `TypeError: len() of a 0-d tensor`。
...
### 15. 添加ptdbg_ascend工具后F.gelu触发ValueError报错：`activation_func must be F.gelu`等。
...
### 16. 添加ptdbg_ascend工具后触发AsStrided算子相关的报错，或者编译相关的报错，如：`Failed to compile Op [AsStrided]`。
...
### 3. 报错：'IsADirectoryError: [Errno 21] Is a directory: '/data/rank0/api_stack_xxx''
...
### 4. 工具报错：AssertionError: Please register hooks to nn.Module
