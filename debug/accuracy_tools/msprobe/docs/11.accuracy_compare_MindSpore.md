...
动态图场景下使用 `mindspore.jit` 装饰特定 Cell 或 function 时，被装饰的部分会被编译成静态图执行。

1. 参见 [动静态图场景L0混合dump](./06.data_dump_MindSpore.md#82-动态图场景)章节, 完成 CPU 或 GPU 与 NPU 的动静态图场景L0混合数据采集。采集的数据文件目录结构示例如下：
    ```lua
    ├── graph
    │   ├── step0
    │   |   ├── rank
