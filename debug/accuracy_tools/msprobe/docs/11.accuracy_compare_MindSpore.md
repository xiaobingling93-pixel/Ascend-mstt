...
| -dm或--data_mapping  | 跨框架比对。通过映射文件指定两个框架中的api或mindspore框架中某个Cell与pytorch中某个Module的具体参数的对应关系。配置该参数的同时需要指定自定义映射文件*.yaml。自定义映射文件的格式请参见[自定义映射文件（data_mapping）](#35-自定义映射文件data_mapping)。| 否       |
...
1. 配置[config.json](../config.json)文件level配置为L2、task配置为tensor或statistics。
...
在采集的等级为mix时，dump数据中既包括api数据也包括cell数据，可以参照[自定义映射文件（data_mapping）](#35-自定义映射文件data_mapping)配置映射关系，也可以通过配置layer_mapping从Layer层识别整网的API和Cell，简化配置。
