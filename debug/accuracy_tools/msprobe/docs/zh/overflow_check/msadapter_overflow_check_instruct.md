...
溢出检测任务的配置示例见["**MindSpore 动态图场景 task 配置为 overflow_check**"](../dump/config_json_examples.md#33-task-配置为-overflow_check)小节。


## 1 接口介绍

溢出检测功能提供的接口与数据采集任务一致，详见[MSAdapter 场景的精度数据采集](../dump/msadapter_data_dump_instruct.md)中的"**2 接口介绍**"小节。

需要注意，目前暂不支持 "L1" level 下 primitive op 的溢出检测。

## 2 示例代码

溢出检测功能使用方式与数据采集任务一致，详见详见[MSAdapter 场景的精度数据采集](../dump/msadapter_data_dump_instruct.md)中的"**3 示例代码**"小节。

## 3 溢出检测结果文件介绍

溢出检测结果文件目录结构与含义与数据采集任务一致，但仅保存溢出 API 或 模块 的真实数据或统计信息。详见详见[MSAdapter 场景的精度数据采集](../dump/msadapter_data_dump_instruct.md)中的"**4 dump 结果文件介绍**"小节。