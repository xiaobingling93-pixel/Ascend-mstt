...
执行精度比对操作需要安装msprobe工具。详见《[msprobe 工具安装指南](../msprobe_install_guide.md)》。
...
1. 参见《[MindSpore 场景的精度数据采集](../dump/mindspore_data_dump_instruct.md)》完成不同环境下MindSpore静态图精度数据的采集，得到不同框架版本的API dump数据。
...
4. 查看比对结果，请详见PyTorch目录下的《[PyTorch 场景的精度比对-精度比对结果分析](./pytorch_accuracy_compare_instruct.md#3-精度比对结果分析)》章节。

### 2.3 不同版本下的全量kernel比对

1. 参见《[MindSpore 场景的精度数据采集](../dump/mindspore_data_dump_instruct.md)》完成不同环境下MindSpore静态图精度数据的采集，得到不同框架版本的kernel dump数据。
...
2. 参见《[MindSpore 场景的精度数据采集](../dump/mindspore_data_dump_instruct.md)》完成不同环境下MindSpore动态图精度数据的采集，得到不同框架版本的cell模块dump数据。
...
5. 查看比对结果，请详见PyTorch目录下的《[PyTorch 场景的精度比对-精度比对结果分析](./pytorch_accuracy_compare_instruct.md#3-精度比对结果分析)》章节。

### 2.5 跨框架的API比对

1. 配置[config.json](../../../config.json)文件level配置为L1、task配置为tensor或statistics。

2. 参见《[MindSpore 场景的精度数据采集](../dump/mindspore_data_dump_instruct.md)》和《[PyTorch 场景的精度数据采集](../dump/pytorch_data_dump_instruct.md)》完成不同环境下API精度数据的采集，得到两个框架的API dump数据。
...
2. 参见《[MindSpore 场景的精度数据采集](../dump/mindspore_data_dump_instruct.md)》和《[PyTorch 场景的精度数据采集](../dump/pytorch_data_dump_instruct.md)》完成不同环境下cell模块精度数据的采集，得到两个框架的cell模块dump数据。
...
2. 参见《[MindSpore 场景的精度数据采集](../dump/mindspore_data_dump_instruct.md)》和《[PyTorch 场景的精度数据采集](../dump/pytorch_data_dump_instruct.md)》完成不同环境下API或模块精度数据的采集，得到两个框架的API或模块dump数据。
...
### 2.8 单点数据比对
1. 参见[单点保存工具](../dump/debugger_save_instruct.md)章节完成 CPU 或 GPU 与 NPU 的单点数据采集。
...
4. Pytorch & MindSpore 动态图场景查看比对结果，请详见PyTorch目录下的《[PyTorch 场景的精度比对-精度比对结果分析](./pytorch_accuracy_compare_instruct.md#3-精度比对结果分析)》章节。
MindSpore静态图场景比对结果：
- `result.csv` 文件列出了所有执行精度比对的 单点保存数据 详细信息和比对结果，示例如下：

  ![compare_result](../figures/save_compare_result_sample.png)
具体字段含义同PyTorch目录下的《[PyTorch 场景的精度比对-精度比对结果分析](./pytorch_accuracy_compare_instruct.md#3-精度比对结果分析)》章节。

### 2.9 动静态图场景L0混合dump数据比对

1. 参见 [msprobe工具MindSpore场景精度数据采集指南](../dump/mindspore_data_dump_instruct.md)，执行dump操作。<br>动态图场景下使用 `mindspore.jit` 装饰特定 Cell 或 function 时，被装饰的部分会被编译成静态图执行。采集的数据文件目录结构示例如下：
    ```lua
    ├── graph
    │   ├── step0
    │   |   ├── rank
...
### 2.10 首差异算子节点识别
参见《[PyTorch 场景的精度比对-首差异算子节点识别](./pytorch_accuracy_compare_instruct.md#215-首差异算子节点识别场景)》章节。
...
- ms_api/pt_api：分别为MindSpore和PyTorch框架的API名称，配置格式为{api_type}.{api_name}。API名称请分别从《[MindSpore 场景的精度数据采集](../dump/mindspore_data_dump_instruct.md)》和《[PyTorch 场景的精度数据采集](../dump/pytorch_data_dump_instruct.md)》中的dump.json文件获取。
...
API和模块名称请分别从《[MindSpore 场景的精度数据采集](../dump/mindspore_data_dump_instruct.md)》和《[PyTorch 场景的精度数据采集](../dump/pytorch_data_dump_instruct.md)》中的dump.json文件获取。
