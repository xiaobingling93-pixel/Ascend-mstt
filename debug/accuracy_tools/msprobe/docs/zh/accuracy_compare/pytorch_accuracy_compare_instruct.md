...
- 本节主要介绍通过命令行和比对函数的方式进行 CPU 或 GPU 与 NPU 的精度数据比对，执行精度比对操作前需要先完成 CPU 或 GPU 与 NPU 的精度数据 dump，参见 [PyTorch 场景的精度数据采集](../dump/pytorch_data_dump_instruct.md)章节。
...
1. 参见[PyTorch 场景下的数据采集](../dump/pytorch_data_dump_instruct.md)章节完成 CPU 或 GPU 与 NPU 的精度数据 dump。
...
2. 参见[PyTorch 场景下的数据采集](../dump/pytorch_data_dump_instruct.md)章节完成 CPU 或 GPU 与 NPU 的精度数据 dump。
...
API和模块名称请从《[PyTorch 场景的精度数据采集](./dump/pytorch_data_dump_instruct.md)》中的dump.json文件获取。
