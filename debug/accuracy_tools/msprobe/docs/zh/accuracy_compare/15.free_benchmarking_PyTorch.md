...
* 该工具的约束
    * 仅支持Pytorch2.x场景；
    * 推荐限定算子范围使用（白名单，或脚本中工具启用范围），算子范围越大，性能和显存损耗越大；详情见[无标杆比对功能在 PyTorch 场景的性能基线报告](../baseline/S02.report_free_benchmarking_validation_performance_baseline.md)
...
![alt text](../figures/free_benchmark_framework.png)
...
### 3.1 安装msprobe工具
参照[msprobe安装](../msprobe_install_guide.md)
### 3.2 config.json配置
修改[config.json](../../../config.json) 的task类型为"**free_benchmark**"开启无标杆功能。支持的配置项详见[配置文件介绍](../dump/config_json_introduct.md)，默认配置如下：
```json
{
    "task": "free_benchmark",
    "dump_path": "./dump_path", 
