...
dump "statistics"模式的性能膨胀大小"与"tensor"模式采集的数据量大小，可以参考[dump基线](../baseline/pytorch_data_dump_perf_baseline.md)。
...
加工具后loss/gnorm发生变化：可能是工具中的item操作引入同步，pt/ms框架的hook机制等原因导致的，详见《[工具导致计算结果变化](../faq.md/#模型计算结果改变原因分析)》。
...
3. 其他参数均在 config.json 文件中可配，详细配置可见 [config.json 介绍](./config_json_introduct.md)。
...
<br>对于复杂模型，如果仅需要监测一部分(如model.A，model.A extends torch.nn.Module)，传入需要监测的部分(如model.A)即可。  
...
3.api_prefix: [dump.json](#31-dumpjson文件说明) 中 api 名的前缀，可选，默认为包名的字符串格式, 即 "torch"。
...
* `dump.json`： 保存API或Module前反向数据的统计量信息。包含dump数据的API名称或Module名称，各数据的dtype、 shape、max、min、mean、L2norm（L2范数，平方根）统计信息以及当配置summary_mode="md5"时的CRC-32数据。具体介绍可参考[dump.json文件说明](#31-dumpjson文件说明)。
