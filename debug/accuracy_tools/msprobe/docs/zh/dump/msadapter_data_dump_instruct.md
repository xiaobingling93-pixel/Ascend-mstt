...
删除 API 的场景：部分模型代码逻辑会存在 API 原生类型校验，工具执行dump操作时，对封装后的模型 API 可能与模型的原生 API 类型不一致，此时可能引发校验失败，详见《[FAQ](../faq.md#异常情况)》中“异常情况”的第10和11条。
...
2. 其他参数与 [config.json](../../../config.json) 文件中的同名配置字段含义相同，具体可见 [config.json 介绍](./config_json_introduct.md)。当参数值非None时，优先级高于  [config.json](../../../config.json) 文件中的同名配置。
...
* `dump.json`： 保存 API 或 Module 前反向数据的统计量信息。包含 dump 数据的 API 名称或 Module 名称，各数据的 dtype、 shape、max、min、mean、L2norm（L2范数，平方根）统计信息以及当配置 summary_mode="md5" 时的 CRC-32 数据。具体介绍可参考[dump.json文件说明](#41-dumpjson-文件说明)。
