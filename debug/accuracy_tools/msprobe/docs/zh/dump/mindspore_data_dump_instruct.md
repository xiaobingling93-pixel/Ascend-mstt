...
采集方式请参见[示例代码 > 静态图场景](#71-静态图场景)。详细介绍请参见《[config.json 配置文件介绍](./config_json_introduct.md#11-通用配置)》中的“level 参数”和《[config.json 配置示例](./config_json_examples.md#2-mindspore-静态图场景)》中的“MindSpore 静态图场景”。
...
详细介绍请参见《[config.json 配置文件介绍](./config_json_introduct.md#11-通用配置)》中的“level 参数”。
...
1. config_path：指定 dump 配置文件路径，string 类型。参数示例："./config.json"。未配置该路径时，默认使用 [config.json](../../../config.json) 文件的默认配置，配置选项含义可见 [config.json 介绍](./config_json_introduct.md)。
2. 其他参数均在 [config.json](../config.json) 文件中可配，详细配置可见 [config.json 介绍](./config_json_introduct.md)。
...
具体使用样例可参考：[单点保存工具使用介绍](./debugger_save_instruct.md)。
...
3.api_prefix: [dump.json](#92-dumpjson-文件说明) 中 api 名的前缀，可选，默认为包名的字符串格式， 即 "torch"。
...
* `dump.json`： 保存API或Cell前反向数据的统计量信息。包含dump数据的API名称或Cell名称，各数据的dtype、 shape、max、min、mean、L2norm（L2范数，平方根）统计信息以及当配置summary_mode="md5"时的CRC-32数据。具体介绍可参考[dump.json文件说明](#92-dumpjson-文件说明)。
