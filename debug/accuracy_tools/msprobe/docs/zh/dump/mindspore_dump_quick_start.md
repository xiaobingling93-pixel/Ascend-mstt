...
```
以上配置参数详细介绍和使用请参见《[config.json 配置文件介绍](./config_json_introduct.md)》和《[config.json 配置示例](./config_json_examples.md#3-MindSpore-动态图场景) 》中的“MindSpore动态图场景”。
...
在 `dump_path` 参数指定的路径下（本例中为 `./output`），会出现如下目录结构，后续精度数据分析操作可使用 msprobe 工具的精度预检和精度比对等功能，详细流程请参见《[msprobe使用手册](../../../README.md)》。
