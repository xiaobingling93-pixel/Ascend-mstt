...
| level             | dump 级别，str 类型，根据不同级别采集不同数据。可选参数：<br/>"L0"：dump 模块级精度数据，使用背景详见 [1.1.1 模块级精度数据 dump 说明](#111-模块级精度数据-dump-说明)。<br/>"L1"：dump API 级精度数据，默认值，仅 PyTorch、MSAdapter 以及 MindSpore 动态图场景支持。<br/>"L2"：dump kernel 级精度数据，PyTorch 场景详细介绍见 [PyTorch 场景的 kernel dump 说明](./pytorch_kernel_dump_instruct.md)；MindSpore 动态图场景详细介绍见 [MindSpore 动态图场景的 kernel dump 说明](./mindspore_kernel_dump_instruct.md)；MindSpore 静态图场景详细介绍见《MindSpore 场景的数据采集》中的 ["**8.1 静态图场景**"](./06.data_dump_MindSpore.md#81-静态图场景)小节。<br/>"mix"：dump module 模块级和 API 级精度数据，即"L0"+"L1"，仅 PyTorch、MSAdapter 以及 MindSpore 动态图场景支持。<br/>"debug"：单点保存功能，详见[单点保存工具](./debugger_save_instruct.md)。<br/>  **配置示例**："level": "L1"。            | 否 |
...
**示例**：
 - [PyTorch场景](config_json_examples.md#11-task-配置为-statistics)
 - [MindSpore静态图场景](config_json_examples.md#21-task-配置为-statistics)
 - [MindSpore动态图场景](config_json_examples.md#31-task-配置为-statistics)
...
**示例**：
 - [PyTorch场景](config_json_examples.md#12-task-配置为-tensor)
 - [MindSpore静态图场景](config_json_examples.md#22-task-配置为-tensor)
 - [MindSpore动态图场景](config_json_examples.md#32-task-配置为-tensor)
...
**示例**：
 - [PyTorch场景](config_json_examples.md#14-task-配置为-overflow_check)
 - [MindSpore静态图场景](config_json_examples.md#23-task-配置为-overflow_check)
 - [MindSpore动态图场景](config_json_examples.md#33-task-配置为-overflow_check)
...
**示例**：
 - [PyTorch场景](config_json_examples.md#15-task-配置为-free_benchmark)
 - [MindSpore动态图场景](config_json_examples.md#34-task-配置为-free_benchmark)

#### 1.6.1 无标杆比对数据存盘格式

无标杆比对在 dump_path 目录下输出结果文件 `free_benchmark.csv`，如下示例：

![free_benchmark](../figures/free_benchmark.png)
...
### 1.7 task 配置为 grad_probe
   ...
   **参数说明**
   ...
   | param_list             | 权重名称列表，表示需要监测的权重。列表为空就表示监测所有权重。默认为空。 | List[str] | 否       |
...
**示例**：
 - [PyTorch场景](config_json_examples.md#16-task-配置为-structure)
 - [MindSpore动态图场景](config_json_examples.md#35-task-配置为-structure)
...
**示例**：
 - [MindSpore动态图场景](config_json_examples.md#36-task-配置为-exception_dump)
 - [MindSpore静态图场景](config_json_examples.md#24-task-配置为-exception_dump)