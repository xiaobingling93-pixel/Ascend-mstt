...
| 异常告警规则     |解释| rule_name | args是否可选        |
|------------|----|-----------|-----------------|
| 梯度异常时序告警   |将当前数值与历史均值比较。如果相对偏差超过阈值，会在打屏信息中提示用户| AnomalyTurbulence | 否，必须传入threshold |
| nan值/极大值告警 |根据是否提供threshold来判断nan值或极大值| AnomalyNan  | 是， 若未配置args或未配置threshold，则默认检测nan，若提供threshold，则检测nan值以及超过阈值的极大值             |
