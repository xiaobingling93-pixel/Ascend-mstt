...
- 启动：
  首次监控：查看config.json文件中dynamic_on字段，若为true则在下一步开启监控。
  非首次监控：查看config文件时间戳，若时间戳更新且config.json文件中dynamic_on字段为true则在下一步开启监控。
- 停止：
  到达collect_times之后自动停止并改config.json文件中dynamic_on字段为false，可再通过上述操作重启。

大部分情况下，用户可在看到异常趋势后再手动更新config.json文件并设dynamic_on为true；此外，使用时若想要在一开始就启动监控，可直接设dynamic_on为true做基础配置的监测（首次不要求时间戳更新）
...
| 场景                                            | 操作方式                                                                                                                                                              | 预期结果                                                            |
|-----------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------|
| 场景1: 使用原始默认的静态模式                              | ```export DYNAMIC_MONITOR=False  ``` 或不设置                                                                                                                         | 走默认分支保存，不受config.json中dynamic_on影响                              |
| 场景2: 进入动态启停模式，一开始不想监控                         | 1.```export DYNAMIC_MONITOR=True``` <br/> 2.config.json中```dynamic_on: false```或不设置                                                                               | 无监控，不保存                                                         |
| 场景3: 进入动态启停模式，一开始就想启动监控                       | 1.```export DYNAMIC_MONITOR=True``` <br/> 2.config.json中```dynamic_on: true```                                                                                    | 训练开始根据config初始配置在下一步（从0计数时对应为第1步）开启监控并保存，采集collect_times次数后结束监控 |
| 场景4: 进入动态启停模式，一开始不想监控，但想在训练中途启动监控             | 1.```export DYNAMIC_MONITOR=True``` <br/> 2.一开始config.json中```dynamic_on: false```或不设置<br/>3.中途改```dynamic_on: true```                                            | 训练中途根据config最新配置在下一步开启监控并保存，采集collect_times次数后结束监控                                     |
| 场景5: 进入动态启停模式，在监控还未结束时中途修改config.json采集配置     | 1.```export DYNAMIC_MONITOR=True``` <br/> 2.期间```dynamic_on: true```启动采集<br/>3.在采集还未达到collect_times次数未结束前，中途修改config.json中采集配置                                    | 之前采集按照旧配置保存，修改后下一步以更新的config.json采集且collect_times重新按照最新配置从0开始计数。此功能可配合中途collect_times改0来实现提前停止监控
| 场景6: 进入动态启停模式，在根据collect_times结束监控后，又需要重新启动监控 | 1.```export DYNAMIC_MONITOR=True``` <br/> 2.期间```dynamic_on: true```启动采集<br/>3.采集达到collect_times次数后结束，程序会自动改```dynamic_on:false```<br/>4.重新改```dynamic_on:true```重启监控 | 之前采集按照旧配置保存，中途停止监控后无保存，重启后下一步以最新config.json重启采集且collect_times重新按照最新配置从0开始计数
...
| "cc_distribution"       | 可选     | 其中"enable"字段控制通信监控模块的开关，仅支持在多卡训练时开启；需要监控通信算子时，务必尽量早地实例化`TrainerMon`, 因为监控通过劫持原始func后挂hook实现，部分加速库初始化时会保存原始function，避免监控失效。"cc_codeline"字段指定监控的代码行，如:`train.py\\[23\\]`，默认为空列表，不特别指定；"cc_pre_hook"字段控制是否监控通信前的数据； 模块会在第二个optimize.step之前打印通信日志，包括通信api的调用栈、输入dtype、通信group。 "cc_log_only"为true时，仅打印日志，不监控通信的输入输出，并在打印后中断训练。可以根据通信日志设置"cc_codeline"，规避与训练过程不相关的通信，比如一些时间、metrics的同步。 |
