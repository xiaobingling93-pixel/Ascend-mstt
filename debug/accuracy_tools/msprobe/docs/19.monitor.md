...
| 字段名字                                                       | 是否必选    | 解释   |
| ------------------------------------------------------------ | -------- | -------- |
|"targets"| 可选 |指定需要监控的模型层， 例如transformer的第0层language_model.encoder.layers.0。如果不清楚模型结构， 可以将 "print_struct" 字段设置为 true， 监控工具会打印模型中torch module的名字和详细结构，并在第1个step后退出。未配置时默认为全量监控。|
...
|"ops"| 可选 |类型为list，与ur_distribution、xy_distribution、mv_distribution、wg_distribution、mg_direction、cc_distribution配合，监控所选张量的统计指标，目前支持"min"、"max"、"norm"、"mean"、"zeros"、"nans"。其中，zeros代表监控所选张量的元素小于eps的比例，nans代表张量中nan的数量。未配置ops时，默认为空列表，表示不采集任何指标。|
...
1、未使用megatron、deepspeed框架时，如何使用monitor监控工具：
...
monitor = TrainerMon(...)
monitor.set_wrapped_optimizer(optimizer)
monitor.monitor_gnorm_with_ad(....optimizer=optimizer)
```

此时optimizer为Optimizer，与使用deepspeed相比，减少一层.optimizer
