...
在激活值监控任务中，all_xy与targets同时生效，all_xy配置为true时，若targets配置module_xx和输入输出对象，则module_xx按targets配置生效，其余module则监控全部输入输出，其中包含input、output、input_grad、output_grad。
