...
- MindSpore场景：mindspore不低于**2.4**，仅支持**MindSpore动态图**
...
| "switch"                | 可选     | 在动态启停时使用，true代表打开监控，false代表关闭监控，默认值为false，且达到collect_times之后会自动将该值置为false待下次改true重启。**仅PyTorch场景支持此参数**。 |
| "collect_times"         | 可选     | 设置采集次数，达到该次数后停止监控，默认值为100000000，目的是一直采集。**仅PyTorch场景支持此参数**。 |
| "start_step"            | 可选     | 设置开始采集step，模型训练达到start_step后开始监控采集，默认值为0，表示从step0开始监控采集。**仅PyTorch场景支持此参数**。 |
| "step_interval"         | 可选     | 设置采集step间隔，默认值为1，表示每个step均采集监控数据。**仅PyTorch场景支持此参数**。 |
