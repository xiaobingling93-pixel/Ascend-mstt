...
| 配置项 | 类型 | 说明 | 示例值 |
|--------|------|------|--------|
| **l2_targets.attention_hook** | List[str] | 指定需要监控的注意力层, 采集"entropy"和"sorftmax_max"指标，需要通过[打印模型结构功能](#打印模型结构)获取 | `["0:0.self_attention.core_attention.flash_attention"]` |
| **l2_targets.linear_hook** | List[str] | 指定需要监控的线性层, 采集"sr"和 "kernel_norm"指标，需要通过[打印模型结构功能](#打印模型结构)获取，支持传入空列表自动识别线性模块 | `["0:0.self_attention.linear_qkv", "0:1.self_attention.linear_qkv"]` |
| **recording_l2_features** | bool | 是否开启L2层特征数据采集 | `true` |
| **sa_order** | str | 计算attention_hook内指标时，指定Attention输入(Q，K)的张量维度排列顺序，支持"s,b,h,d"和"b,s,h,d", 默认为"s,b,h,d" | `"s,b,h,d"` |
