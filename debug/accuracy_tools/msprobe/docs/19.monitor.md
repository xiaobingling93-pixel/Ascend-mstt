...
本项目开发了一个模型训练状态监控工具，能够收集和聚合模型训练过程中的网络层，优化器， 通信算子的中间值，实时呈现训练状态。
...
- 打印结构  
工具提供可选项"print_struct"帮助打印模型结构。工具会在在第一个step后打印结构并停止进程。  
```json
{
    "print_struct": true
}
...
- 输出路径  
通过环境变量`KJ600_OUTPUT_DIR`设置，默认为"kj600_output"。
```shell
...
|"ndigits"| 可选 |"format"为"api"时，蛇者落盘文件中的小数位数，默认为6|
