...
### 异常告警（支持梯度异常时序告警或nan值/极大值告警）
1. 训练前配置相关参数

工具支持自动判断训练过程中的异常现象，当前支持梯度异常时序告警或nan值/极大值告警，需要在配置文件中设置alert相关字段。
- 梯度异常时序告警配置规则："AnomalyTurbulence"会将当前数值与历史均值比较。如果相对偏差超过阈值，会在打屏信息中提示用户。如果打开"`dump`"选项，则会将异常梯度相关信息落盘到目录`monitor_output/anomaly_detected`，用于后续时序判断。
```json
    "alert": {
...
```
- nan值/极大值告警配置规则："AnomalyNan"会根据是否提供threshold来判断nan值或极大值。若未配置args或未配置threshold，则默认检测nan，若提供threshold，则检测nan值以及超过阈值的极大值。如果检测到相关异常，会在打屏信息中提示用户。如果打开"`dump`"选项，则会将异常梯度相关信息落盘到目录`monitor_output/anomaly_detected`，用于后续时序判断。
```json
    "alert": {
