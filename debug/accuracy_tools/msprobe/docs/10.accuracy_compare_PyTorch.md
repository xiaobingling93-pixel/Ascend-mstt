...
| stack_mode   | 配置 stack_mode 的开关，bool 类型。仅当配置 stack_json_path 时需要开启。当不配置stack_json_path 时，自动识别是否存在stack.json并决定是否呈现NPU_Stack_Info。<br/>**配置示例**：stack_mode=True，默认为 False。                                                                                                                                                                                                                                                                                                                            | 否       |
...
| **kwargs       | 支持 compare 的所有可选参数。 多卡场景，stack_mode不生效，自动识别是否存在stack.json并决定是否呈现NPU_Stack_Info；单卡场景见2.2.1参数说明中的stack_mode。                                                | 否       |
...
| 参数名                 | 说明                                                         | 是否必选 |
| ---------------------- | ------------------------------------------------------------ | -------- |
| -i 或 --input_dir      | 多卡比对结果存盘目录，即使用compare比对的结果输出目录，str类型。 | 是       |
