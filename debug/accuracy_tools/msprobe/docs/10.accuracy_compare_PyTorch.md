...
| is_print_compare_log | 配置是否开启单个算子的日志打屏。可取值 true 或 false，默认为 true。关闭后则只输出常规日志。数据类型：bool | 否                 |
...
精度比对结果 `compare_result_{timestamp}.xlsx` 文件中对于存在异常信息的 API 会进行高亮处理：

- 红色可能出现的情况有：
  - NPU max 或 NPU min 信息中存在 nan/inf；
  - Max diff 存在大于 1e+10 的值；
  - 统计数据中 output 的 (Max diff) / max(0.01, Bench max) > 0.5；
  - 真实数据中 One Thousandth Err Ratio 的 input > 0.9 同时 output < 0.6。
- 黄色可能出现的情况有：
  - Max diff 的 input 与 output 都大于 1，同时 output 比 input 大一个数量级以上；
  - 统计数据 Max diff 除以 max(0.01, Bench max) 的 output > 0.1 同时 input < 0.01；
  - 真实数据 One Thousandth Err Ratio 的 input - output > 0.1；
  - 真实数据 Cosine 的 input - output > 0.1。
