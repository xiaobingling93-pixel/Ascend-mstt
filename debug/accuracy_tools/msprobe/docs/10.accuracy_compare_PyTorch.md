...
| 参数名               | 说明                                                                                                                                                                                  | 是否必选 |
| -------------------- |-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------|
| npu_path             | 配置 NPU 环境下的 dump.json/debug.json 文件（单卡场景）或真实数据目录（多卡场景），str 类型。                                                                                                                                 | 是    |
| bench_path           | 配置 CPU、GPU 或 NPU 环境下的 dump.json/debug.json 文件（单卡场景）或真实数据目录（多卡场景），str 类型。                                                                                                                       | 是    |
...
2. 参见 [单点保存工具](./28.debugger_save_instruction.md)章节完成 CPU 或 GPU 与 NPU 的单点数据采集。

3. 创建[比对文件（单点数据）](#217-比对文件单点数据)。
