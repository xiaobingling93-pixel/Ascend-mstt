...
**配置文件参数说明**
   ...
   | api_name             | 算子名（目前MindSpore支持列表为：Mint，Tensor，Msadapter支持类型包括：Tensor，Functional，Torch类中可自动求导api），如Mint.split.1，Functional.softmax.3、Tensor.add.0、Torch.matmul.5等。 | 
