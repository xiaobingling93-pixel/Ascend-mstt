...
| 参数名        | 解释                 | 是否必选 |
| ------------- | ---------------------- | -------- |
| overflow_nums | 控制溢出次数，int 类型，仅 PyTorch 场景支持，表示第 N 次溢出时，停止训练，过程中检测到溢出 API 对应的 kernel 数据均 dump。<br/>**配置示例**："overflow_nums": 3。默认为 1，即检测到1次溢出，训练停止，配置为 -1 时，表示持续检测溢出直到训练结束。 | 否       |
