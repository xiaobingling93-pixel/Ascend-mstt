...
| if_preheat   | 预热功能（仅 PyTorch 场景支持），开启功能后工具可以根据每次迭代的输出调整精度算法的阈值，从而更准确地找出存在精度问题的 API，bool 类型。可选参数：<br/>  true（开启）或 false（关闭），默认关闭。<br/>  **配置示例**："if_preheat": "true"。</br>"handler_type": "fix"不支持预热。 | 否       |
