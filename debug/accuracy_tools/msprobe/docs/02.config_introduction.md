...
| level             | dump 级别，str 类型，根据不同级别采集不同数据。可选参数：<br/>"L0"：dump 模块级精度数据，仅 PyTorch 与 MindSpore 动态图场景支持，使用背景详见 [1.1.1 模块级精度数据 dump 说明](#111-模块级精度数据-dump-说明)；<br/>"L1"：dump API 级精度数据，默认值，仅 PyTorch 与 MindSpore 动态图场景支持；<br/>"L2"：dump kernel 级精度数据，PyTorch场景详细介绍见 [PyTorch 场景的 kernel dump 说明](./04.kernel_dump_PyTorch.md)；<br/>"mix"：dump module 模块级和 API 级精度数据，即"L0"+"L1"，仅 PyTorch 与 MindSpore 动态图场景支持。<br/> "debug": 单点保存场景，配合PrecisionDebugger.save接口使用。配置文件约束详见[单点保存功能使用说明](#112-单点保存功能使用说明) <br/> **配置示例**："level": "L1"。                                                                                                                                                    | 否 |
...
L0, L1, mix dump存在盲区，网络中的非api/module的输入输出不会被批量dump下来。单点保存提供类似np.save和print的功能和使用体验，可以保存指定的变量。并且针对大模型场景提供增强的特性：1）可保存变量的反向梯度结果。2）可直接保存嵌套结构数据（list，dict），无需手动遍历。3）自动分rank保存，多次调用自动计数。4）可配置保存统计值或者张量。

单点保存功能配置文件说明：
- 公共配置项中除"level", "task", "rank", "dump_path"外，其余配置项不生效。
- "level"配置为"debug"开启单点保存功能。
- "task"仅支持配置为"statistics"或者"tensor"，表示保存统计值/张量。
- "statistics"任务子配置项中只有"summary_mode"生效，支持"statistics"和"md5"。
- "tensor"任务的子配置项都不生效。

单点保存功能落盘文件说明：
- "task"配置为"statistics"场景，在dump目录下生成包含变量统计值信息的debug.json文件。
- "task"配置为"tensor"场景，除了在dump目录下生成包含变量统计值信息的debug.json文件，还在dump子目录dump_tensor_data中保存了张量二进制文件，文件格式为```{$variabel}```
