...
**功能说明**：固定随机种子和确定性。在工具导入之后的位置添加。

**原型**：

```Python
seed_all(mode, seed=1234)
```

1. mode：是否开启确定性计算，bool类型，True表示开启确定性计算，False表示不开启确定性计算
2. seed: 随机种子，int类型，默认为1234
