...
### 2.5 跨文件采集数据
为了确保所有API都被工具封装，PrecisionDebugger的实例化通常放在训练工程的入口位置。但有的时候，需要采集另一个文件中模型的数据。
假设有两个文件，train.py和module.py。train.py为训练工程入口，module.py为模型定义文件。
本示例介绍了如何采集module.py中定义的ModuleOP模块中某些子模块或API的前反向数据。
