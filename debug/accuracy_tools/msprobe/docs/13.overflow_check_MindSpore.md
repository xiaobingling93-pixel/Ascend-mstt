...
msprobe 工具提供静态图O2编译等级下的过程溢出检测与动态图场景下的结果溢出检测。其中前者检测对象为 kernel 级别，对应 config.json 配置中的 "L2" level，后者检测对象为 API 级别(支持的API类型为ops,Tensor，mint或mint.nn.functional,不支持Primitive,Jit类API)，对应 config.json 配置中的 "L1" level。
