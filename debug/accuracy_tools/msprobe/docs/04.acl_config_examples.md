...
```json
{
	...
	"acl_config": "./xxx/acl_config.json"
}
```
...
```json
{
	...
	"tensor": {
		...
		"list":[],
		"data_mode": ["all"],
		"backward_input": ["./xxx/Tensor.__mul__.10.backward.input.0.pt"],
		"file_format": "npy"
	},
	"acl_config": "./xxx/acl_config.json"
}
...
| dump_path      | dump 数据文件存储到运行环境的目录，主要配置的是 kernel 级数据的存放路径。支持配置绝对路径或相对路径。dump_path 须为已存在目录。注意该路径必须填写，且须与config.json中的dump_path保持一致。 |
| dump_mode      | dump 数据模式，可取值，<br/> output：dump API 的输出数据，为默认值；<br/> input：dump API 的输入数据；<br/> all：dump API 的输入、输出数据。 |
| dump_op_switch | 单 API 模型 dump 数据开关，可取值，<br>        off：关闭单 API 模型 dump，为默认值；<br>        on：开启单 API 模型 dump。 |
