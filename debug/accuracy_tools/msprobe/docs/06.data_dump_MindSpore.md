...
dump 的"statistics"模式性能膨胀大小"与"tensor"模式采集数据量大小，可以参考[dump基线](data_dump_MindSpore/data_dump_MindSpore_baseline.md)。
...
动态图场景下使能 PSJit 或 PIJit（MindSpore 自 2.6 版本起将其更名为 Ast 或 Bytecode，但为了保持术语一致，下文仍沿用原有称谓），装饰特定 Cell 或 function，被装饰的部分会全部/部分使能**静态图**流程。
