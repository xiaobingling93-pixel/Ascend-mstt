...
|版本|特性|
|-------|------|
|1.0.4|【数据采集】</br>1. 支持 config.json 中的 step 和 rank 传入范围；</br>2. 优化了 MindSpore 场景的 step 机制，step 结束后训练继续运行。</br>【精度预检】</br>1. 在 PyTorch 场景，支持部分 NPU 融合算子预检。</br>【精度比对】</br>1. 解决了使用 MindSpore 需要安装 PyTorch 的问题。</br>【无标杆比对】</br>1. 补充在 PyTorch 场景的性能基线报告；</br>2. 支持 MindSpore 场景的 change_value 扰动模式。|
|1.0.3|【精度预检】</br>1. 落盘数据小；</br>2. 支持随机生成模式和真实数据模式；</br>3. 单 API 测试，排除整网中的累计误差问题。</br>【梯度检测】</br>1. 使用便捷，无需在训练流程里插入代码；</br>2. 可以精准定位问题出现的 step。 |
...
Location: /home/xxx/miniconda3/envs/xxx/lib/python3.8/site-packages/mindstudio_probe-1.0.x-py3.8.egg
