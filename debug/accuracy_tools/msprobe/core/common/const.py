...
class CompareConst:
    ...
    SUMMARY_COMPARE_RESULT_HEADER = [
        NPU_NAME, BENCH_NAME, "NPU Tensor Dtype", "Bench Tensor Dtype", NPU_SHAPE, BENCH_SHAPE, MAX_DIFF, MIN_DIFF,
        MEAN_DIFF, NORM_DIFF, NPU_MAX, NPU_MIN, NPU_MEAN, NPU_NORM, BENCH_MAX, BENCH_MIN, BENCH_MEAN, BENCH_NORM,
        RESULT, ERROR_MESSAGE
    ]       # TODO 用的老比对表格，测试用，测试后还原

    MD5_COMPARE_RESULT_HEADER = [
        NPU_NAME, BENCH_NAME, NPU_DTYPE, BENCH_DTYPE, NPU_SHAPE, BENCH_SHAPE, NPU_MD5, BENCH_MD5, RESULT
    ]

    COMPARE_RESULT_HEADER_STACK = COMPARE_RESULT_HEADER.append(STACK)

    SUMMARY_COMPARE_RESULT_HEADER_STACK = SUMMARY_COMPARE_RESULT_HEADER.append(STACK)

    MD5_COMPARE_RESULT_HEADER_STACK = MD5_COMPARE_RESULT_HEADER.append(STACK)   # TODO 待新比对表格测试确认
    ...
    #                          MAX_RELATIVE_ERR, MIN_RELATIVE_ERR, MEAN_RELATIVE_ERR, NORM_RELATIVE_ERR] # TODO 测试后用这个替换
    SUMMARY_COMPARE_INDEX = [MAX_DIFF, MIN_DIFF, MEAN_DIFF, NORM_DIFF]
