...
def task_dumppath_get(input_param):
    ...
    if npu_json_data['task'] == Const.TENSOR:
        ...
        raise CompareException(CompareException.INVALID_TASK_ERROR)
    input_param['npu_dump_data_dir'] = os.path.dirname(npu_json_path) + Const.DUMP_TENSOR_DATA
    input_param['bench_dump_data_dir'] = os.path.dirname(bench_json_path) + Const.DUMP_TENSOR_DATA
