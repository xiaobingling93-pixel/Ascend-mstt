...
2. 执行如下示例命令进行比对：
   ...
   **完整参数说明**
   ...
   | -c或--compare_only | 是否自动精度分析。未配置默认开启，开启后工具自动针对比对结果进行分析，识别到第一个精度不达标节点（在比对结果文件中的“Accuracy Reached or Not”列显示为No），并给出问题可能产生的原因（打屏展示并生成advisor_{timestamp}.txt文件）。该参数默认未配置，表示开启，通过配置该参数关闭自动精度分析，仅输出比对结果表格。 | 否       |
