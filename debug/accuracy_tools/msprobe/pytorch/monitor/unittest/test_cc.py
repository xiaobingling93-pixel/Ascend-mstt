...
def test_gather(context, rank, world_size, async_op):
    ...
    context.aggregate()
    if rank == dst:
        assert_context(context.data,
                       {'pre': [a, torch.zeros(world_size, 2, dtype=DTYPE)], 'post': [a, torch.stack(data)]}, rank)
...
def test_all_gather(context, rank, world_size, async_op):
    ...
    context.aggregate()
    assert_context(context.data,
                   {'pre': [torch.zeros(world_size, DIM, dtype=DTYPE), a], 'post': [torch.stack(data), a]}, rank)
...
def test_reduce_scatter(context, rank, world_size, async_op):
    ...
    assert_context(context.data, {'pre': [torch.zeros_like(a), torch.stack(data)], 'post': [output, torch.stack(data)]},
                   rank)
...
def test_reduce_scatter_tensor(context, rank, world_size, async_op):
    ...
    res = torch.tensor([total] * DIM, dtype=DTYPE, device=f'{DEVICE}:{rank}')
    assert_context(context.data,
                   {'pre': [torch.zeros_like(a, dtype=DTYPE, device=f'{DEVICE}:{rank}'), a], 'post': [output, a]}, rank)
    assert_equal(res, output, rank)
...
def test_scatter(context, rank, world_size, async_op):
    ...
    src = 0
    if rank == src:
        scatter_list = [10 * torch.tensor([i + 1] * DIM, dtype=DTYPE, device=f'{DEVICE}:{rank}') for i in
                        range(world_size)]
    else:
        scatter_list = None
    ...
    if rank == src:
        assert_context(context.data,
                       {'pre': [local_a, torch.stack(scatter_list)], 'post': [a, torch.stack(scatter_list)]}, rank)
    else:
        assert_context(context.data, {'pre': [local_a], 'post': [a]}, rank)
...
def test_batch_isend_irecv(context, rank, world_size, async_op):
    ...
    for req in reqs:
        req.wait()
    context.aggregate()
    assert_context(context.data, {'pre': [torch.stack([send_tensor, torch.zeros_like(send_tensor)])],
                                  'post': [torch.stack([send_tensor, recv_tensor])]}, rank)
    assert_equal(recv_tensor, torch.tensor([(rank - 1 + world_size) % world_size + 1] * DIM, device=f'{DEVICE}:{rank}'),
                 rank)
