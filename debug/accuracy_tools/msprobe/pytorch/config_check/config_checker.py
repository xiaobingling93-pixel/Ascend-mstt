...
from msprobe.pytorch.config_check.checkers.base_checker import PackInput
...
class ConfigChecker:
    ...
    def __init__(self, model=None, shell_path=None, output_zip_path="./config_check_pack.zip"):
    ...
    def compare(bench_zip_path, cmp_zip_path, outpath):
        ...
        bench_dir = os.path.join(outpath, "bench")
        cmp_dir = os.path.join(outpath, "cmp")
        ...
        for checker in ConfigChecker.checkers.values():
            checker_name, pass_check, df = checker.compare_ex(bench_dir, cmp_dir, outpath)
            if checker_name:
        ...
        save_excel(os.path.join(outpath, ConfigChecker.result_filename), result)
        config_checking_print(f"config checking result save to {os.path.realpath(outpath)}")


    def pack(self):
        config_checking_print(f"pack result zip path {os.path.realpath(self.pack_input.output_zip_path)}")
        if dist.is_initialized() and dist.get_rank() == 0:
            config_checking_print(f"pack result zip path {self.pack_input.output_zip_path}")
            if os.path.exists(self.pack_input.output_zip_path):
                os.remove(self.pack_input.output_zip_path)
        ...
        if self.pack_input.model:
            self.pack_input.model.register_forward_pre_hook(hook, with_kwargs=True)
        for checker in ConfigChecker.checkers.values():
            if checker.input_needed and not getattr(self.pack_input, checker.input_needed):
                continue
            if dist.is_initialized() and dist.get_rank() != 0 and not checker.multi_rank:
                continue
