...
class TestGradCSV(unittest.TestCase):
    def test_level_L0_header(self):
        self.assertEqual(['param_name', 'MD5', 'max', 'min', 'norm', 'shape'], 
                         GradStatCsv.generate_csv_header(GradientMonitor.level_adp["L0"], [-1, 0, 1]))

    def test_level_L1_header(self):
        self.assertEqual(['param_name', 'max', 'min', 'norm', 'shape'], 
                         GradStatCsv.generate_csv_header(GradientMonitor.level_adp["L1"], [-1, 0, 1]))

    def test_level_L2_header(self):
        self.assertEqual(['param_name', '(-inf, -1]', '(-1, 0]', '(0, 1]', '(1, inf)', '=0', 'max', 'min', 'norm', 'shape'], 
                         GradStatCsv.generate_csv_header(GradientMonitor.level_adp["L2"], [-1, 0, 1]))

    def test_level_L0_content(self):
        generated_csv_line = GradStatCsv.generate_csv_line("model.conv2d", GradientMonitor.level_adp["L0"], grad_tensor, [-1, 0, 1])
    ...
    def test_level_L1_content(self):
        generated_csv_line = GradStatCsv.generate_csv_line("model.conv2d", GradientMonitor.level_adp["L1"], grad_tensor, [-1, 0, 1])
        self.assertEqual(['model.conv2d', 2.0, -2.0, 2.851315498352051, [2, 2]],
                         generated_csv_line)

    def test_level_L2_content(self):
        generated_csv_line = GradStatCsv.generate_csv_line("model.conv2d", GradientMonitor.level_adp["L2"], grad_tensor, [-1, 0, 1])
