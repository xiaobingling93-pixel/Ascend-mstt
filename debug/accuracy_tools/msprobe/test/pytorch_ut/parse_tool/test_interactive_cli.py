...
class TestInteractiveCli(unittest.TestCase):
    ...
    @patch('msprobe.pytorch.parse_tool.lib.interactive_cli.Util.execute_command', return_value=None)
    def test_default(self, mock_execute_command):
        res = self.interactive_cli.default()

        mock_execute_command.assert_called_once()
        self.assertFalse(res)

    @patch('msprobe.pytorch.parse_tool.lib.interactive_cli.Util.execute_command', return_value=None)
    def test_do_run(self, mock_execute_command):
        self.interactive_cli.do_run()
    ...
    def test_do_vc(self, mock_do_vector_compare, mock_do_compare_converted_dir):
        with patch('msprobe.pytorch.parse_tool.lib.interactive_cli.Util.check_path_valid'), \
            ...
            with patch('msprobe.pytorch.parse_tool.lib.interactive_cli.Util.dir_contains_only', return_value=False):
                self.interactive_cli.do_vc('-m my_dump_path -g golden_dump_path -out output_path -cmp_path msaccucmp_path')
                
                mock_do_vector_compare.assert_called_once()

            with patch('msprobe.pytorch.parse_tool.lib.interactive_cli.Util.dir_contains_only', return_value=True):
                self.interactive_cli.do_vc('-m my_dump_path -g golden_dump_path -out output_path -cmp_path msaccucmp_path')
