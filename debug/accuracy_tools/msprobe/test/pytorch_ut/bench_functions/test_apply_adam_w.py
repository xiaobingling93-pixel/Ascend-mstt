...
class TestNpuApplyAdamW(TestCase):
    def test_npu_apply_adam_w(self):
        ...
        self.assertTrue(torch.allclose(var_out, torch.tensor([0.09992456, 0.19989273, 0.29986808])))
        self.assertTrue(torch.allclose(m_out, torch.tensor([0.09999999, 0.19999999, 0.30000001])))
        self.assertTrue(torch.allclose(v_out, torch.tensor([0.09991000, 0.19983999, 0.29979002])))
