...
class TestStandardConfig(unittest.TestCase):
    def test_get_small_value(self):
        # 测试已定义的数据类型
        self.assertEqual(StandardConfig.get_small_value(torch.float16), 2**-10)
        self.assertEqual(StandardConfig.get_small_value(torch.bfloat16), 2**-10)
        self.assertEqual(StandardConfig.get_small_value(torch.float32), 2**-20)
        
        # 测试未定义的数据类型（应返回默认值）
        self.assertEqual(StandardConfig.get_small_value(torch.int32), 2**-20)
