...
class TestService(unittest.TestCase):
    ...
    def test_start(self):
        with patch("atat.pytorch.service.get_rank_if_initialized", return_value=0), \
                patch("atat.pytorch.service.Service.create_dirs", return_value=None):
            self.service.start(None)
        self.assertEqual(self.service.current_rank, 0)

    def test_stop_and_step(self):
        with patch("atat.core.data_dump.data_collector.DataCollector.write_json", return_value=None):
            self.service.stop()
    ...
    def test_register_hook_new(self):
        ...
        with patch("atat.pytorch.service.logger.info_on_rank_0") as mock_logger, \
                patch("atat.pytorch.service.remove_dropout", return_value=None):
            self.service.register_hook_new()
            self.assertEqual(mock_logger.call_count, 2)

    def test_create_dirs(self):
        with patch("atat.pytorch.service.Path.mkdir", return_value=None), \
                patch("atat.core.common.file_check.FileChecker.common_check", return_value=None), \
                patch("atat.core.data_dump.data_collector.DataCollector.update_dump_paths",
                      return_value=None):
