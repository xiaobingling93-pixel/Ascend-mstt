...
class MultiDataManager(DataManager):
    ...
    def save_results(self, api_name_str):
        """保存结果，线程安全操作"""
        print(f"DEBUG: is_first_write={self.is_first_write}, shared_is_first_write={self.shared_is_first_write.value}")
        with self.lock:  # 确保保存操作不会被多个进程同时进行
            print(f"DEBUG: Writing headers to {self.detail_out_path} and {self.result_out_path}")
