...
class TestImprovePrecisionPerturbation(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        cls.improve_precision_pert = ImprovePrecisionPerturbation("mindspore.ops.add")
    ...
    def test_handle(self, mock_warning):
        ...
        mock_warning.assert_called_with("mindspore.ops.add can not improve precision.")
        self.assertFalse(self.improve_precision_pert.is_fuzzed)
        ...
        self.assertIsNone(params.fuzzed_value)

        TestImprovePrecisionPerturbation.improve_precision_pert.api_name = "mindspore.communication.comm_func.reduce"
        fuzzed_value = [Tensor([1.0], dtype=ms.float32), Tensor([5.0], dtype=ms.float32)]
        self.improve_precision_pert.handle(params)
        self.assertTrue((params.fuzzed_value[0] == fuzzed_value[0]).all())
        self.assertTrue((params.fuzzed_value[1] == fuzzed_value[1]).all())
        self.assertEqual(params.fuzzed_value[0].dtype, fuzzed_value[0].dtype)
        self.assertEqual(params.fuzzed_value[1].dtype, fuzzed_value[1].dtype)
