...
from msprobe.mindspore.compare.npy_compare import compare_ops_apply, get_error_type, reshape_value, get_relative_err, \
    get_error_message
...
def check_type_shape_match(npu_struct, bench_struct):
    ...
    for npu_type_shape, bench_type_shape in zip(npu_struct, bench_struct):
        ...
        if not type_match:
            if [npu_type, bench_type] in [["Float16", "Float32"], ["Float32", "Float16"]]:                    
                type_match = True
            else:
                type_match = False
