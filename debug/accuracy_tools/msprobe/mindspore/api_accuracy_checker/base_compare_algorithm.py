...
class BaseCompareAlgorithm(ABC):
    ...
    def _convert_to_np_float64_ndarray(self, tensor):
        if isinstance(tensor, mindspore.Tensor):
            ndarray = tensor.as_type(mindspore.float64).numpy()
        elif isinstance(tensor, torch.Tensor):
            ndarray = tensor.to(torch.float64, copy=True).numpy()
        else:
            raise ValueError #todo not tensor
        return ndarray

    def _check_two_tensor(self, bench_compute_element, tested_compute_element):
...
class CosineSimilarityCompareAlgorithm(BaseCompareAlgorithm):
    ...
    def check_validity(self, bench_compute_element, tested_compute_element):
        return self._check_two_tensor(bench_compute_element, tested_compute_element)

    def run_compare(self, bench_compute_element, tested_compute_element):
        bench_ndarray = self._convert_to_np_float64_ndarray(bench_compute_element.get_parameter())
        tested_ndarray = self._convert_to_np_float64_ndarray(tested_compute_element.get_parameter())
