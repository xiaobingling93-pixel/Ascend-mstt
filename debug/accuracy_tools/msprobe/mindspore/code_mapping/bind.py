...
def bind_for_statistic(statistic_files: List[str], match_dict: Dict):
    ...
    for statistic_file in statistic_files:
        try:
            ...
            logger.error(f"读取统计文件失败: {statistic_file}, 错误: {e}")
            continue
        ...
        try:
            # 使用write_csv安全写入文件
            write_csv(df, statistic_file, mode="w", header=True, malicious_check=False)
            logger.info("处理完成: %s，代码堆栈已保存至 %s", statistic_file, statistic_file)
        except Exception as e:
            logger.error(f"写入统计文件失败: {statistic_file}, 错误: {e}")
            continue
