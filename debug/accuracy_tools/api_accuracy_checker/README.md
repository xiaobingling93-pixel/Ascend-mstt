...
**标杆比对法**：真实数据模式下，将NPU vs CPU的预检比对结果和GPU vs CPU的预检比对结果进行比对汇总，最终给出判定结果的精度预检方法。
...
| NPU Dtype      | NPU数据的API数据类型。                                       |
| Shape          | API的Shape信息。                                             |
| 余弦相似度     | NPU数据与标杆数据的余弦相似度。                              |
| 最大绝对误差   | NPU数据与标杆数据的最大绝对误差。                            |
| 双百指标       | 双百精度指标。是指NPU的Tensor中的元素逐个与对应的标杆数据对比，相对误差大于百分之一的比例占总元素个数的比例小于百分之一。 |
| 双千指标       | 双千精度指标。是指NPU的Tensor中的元素逐个与对应的标杆数据对比，相对误差大于千分之一的比例占总元素个数的比例小于千分之一。 |
| 双万指标       | 双万精度指标。是指NPU的Tensor中的元素逐个与对应的标杆数据对比，相对误差大于万分之一的比例占总元素个数的比例小于万分之一。 |
| 错误率         | 错误率。                                                     |
| 误差均衡性     | NPU数据与标杆数据转换成低精度后值的上下浮动情况。            |
| 均方根误差     | NPU数据与标杆数据的均方根误差。                              |
| 小值域错误占比 |                                                              |
| 相对误差最大值 | NPU数据与标杆数据相对误差的最大值。                          |
| 相对误差平均值 | NPU数据与标杆数据相对误差的平均值。                          |
