...
class TestAlgorithmMethods(unittest.TestCase):
    ...
    def test_compare_torch_tensor(self):
        ...
        status, compare_column, message = cmp.compare_torch_tensor(cpu_output, npu_output, compare_column)
        self.assertEqual(status, "pass")

    def test_compare_bool_tensor(self):
        cpu_output = np.array([True, False, True])
        npu_output = np.array([True, False, True])
        self.assertEqual(cmp.compare_bool_tensor(cpu_output, npu_output), (0.0, 'pass', ''))
    ...
    def test_cosine_sim(self):
        ...
        result, is_similar, msg = alg.cosine_sim(cpu_output, npu_output)
        self.assertAlmostEqual(result, 1.0, places=14, msg=msg)
    ...
    def test_compare_builtin_type(self):
        ...
        bench_out = 1
        npu_out = 1
        status, compare_result, message = cmp.compare_builtin_type(bench_out, npu_out, compare_column)
